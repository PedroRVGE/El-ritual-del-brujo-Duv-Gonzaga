...
public class ExportadorDot {
    ...
    private static Set<String> pathEdges(List<Integer> path) {
        ...
        for (int i = 0; i + 1 < path.size(); i++) s.add(path.get(i) + "->" + path.get(i + 1));
        return s;
    }
    ...
    private static void generarImagen(String archivoDot, String archivoImagen) {
        try {
            ...
            if (code == 0) System.out.println("Imagen generada: " + archivoImagen);
            else System.err.println("Graphviz (dot) terminó con código " + code);
    ...
    private static void abrirImagen(String archivoImagen) {
        try {
            ...
            if (imagen.exists()) Desktop.getDesktop().open(imagen);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
